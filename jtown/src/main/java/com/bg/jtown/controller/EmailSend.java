...
public class EmailSend {
	...
	public void sendConfirmEmail(String username) {
		...
		sb.append("				Mirros 계정 (" + jtownUser.getName()
				+ ")에 대한 이메일 인증 이메일 입니다.<br/>");
		sb.append("				아래에 링크를 클릭하시면 이메일 인증이 완료 됩니다.");
		sb.append("			</p>");
		sb.append("			<p style='margin: 1em 0;'>");
		sb.append("				<a href='http://www.mirros.net/confirmEmailAddress?id="
				+ username + "&series=" + encryptText
				+ "' target='_self' style='color: #ef662a;'>링크를 클릭해주세요</a>");
		sb.append("			</p>");
		sb.append("			<p style='margin: 1em 0;'>");
	...
	public void sendTempPasswordEmail(String username) {
		...
		sb.append("		</div>");
		sb.append("		<div style='padding: 10px;'>");
		sb.append("			<h1 style='font-weight: normal;'>" + jtownUser.getName()
				+ "님, 비밀번호를 잊으셨나요?</h1>");
		sb.append("			<p style='margin: 1em 0;'>");
		sb.append("				Mirros 계정 (" + jtownUser.getUsername()
				+ ")에 대한 비밀번호 재설정 요청을 받았습니다.<br/>");
	...
	public void sendSellerTempPasswordEmail(Integer pn) {
		...
		JtownUser jtownUser = lp.getJtownUser();
		String username = jtownUser.getUsername();
		String tempPassword = customJdbcUserDetailManager
				.changeTempPassword(username);

		String subject = "[Mirros] 아이디/비밀번호를 알려 드립니다.";
		StringBuffer sb = new StringBuffer();
		sb.append("	<div style='font-family: Dotum, Gulim; font-size: 12px; line-height: 2; width: 600px;'>");
		sb.append("		<div style='border-bottom: 2px solid #ffa500; padding: 10px;'>");
		sb.append("			<a href='https://www.mirros.net'><img alt='Mirros' src='https://www.mirros.net/resources/images/jt-main-banner.png'></a>");
		...
		sb.append("		<div style='padding: 10px;'>");
		sb.append("			<h1 style='font-weight: normal;'>" + jtownUser.getName()
				+ "님, 아이디와 비밀번호를 잊으셨나요?</h1>");
		sb.append("			<p style='margin: 1em 0;'>");
		sb.append("				Mirros 계정 (" + username
				+ ")에 대한 비밀번호 재설정 요청을 받았습니다.<br/>");
		sb.append("				비밀번호가 재설정 되었으며, 재 설정된 비밀번호는 아래와 같습니다.");
		sb.append("			</p>");
		sb.append("			<p style='margin: 1em 0;'>");
		sb.append("				계정 : " + username);
		sb.append("			</p>");
		sb.append("			<p style='margin: 1em 0;'>");
		sb.append("				비밀번호 : " + tempPassword);
		sb.append("			</p>");
		...
		emailSender.sendEmail(email, subject, sb.toString());
	}
}
