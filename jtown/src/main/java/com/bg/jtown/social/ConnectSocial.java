...
public class ConnectSocial {

	private final static Logger logger = LoggerFactory
			.getLogger(ConnectSocial.class);
	 ...
	 */
	@Inject
	public ConnectSocial(ConnectionFactoryLocator connectionFactoryLocator,
	...
	public void addInterceptor(ConnectInterceptor<?> interceptor) {
		Class<?> serviceApiType = GenericTypeResolver.resolveTypeArgument(
				interceptor.getClass(), ConnectInterceptor.class);
	...
	public String connectionStatus(@PathVariable String providerId,
		...
		logger.debug("Connection GET :  ProviederId [ " + providerId + " ] ");
	...
	public RedirectView connect(@PathVariable String providerId,
		...
		logger.debug("Connection POST :  ProviederId [ " + providerId + " ] ");
		...
		logger.debug("Connection POST :  Intercetpor  ");

		try {
			logger.debug("Connection POST :  try  ");

			return new RedirectView(webSupport.buildOAuthUrl(connectionFactory,
					request, parameters));
		} catch (Exception e) {
			logger.debug("Connection POST :  catch ");
	...
	public RedirectView oauth1Callback(@PathVariable String providerId,
		...
		logger.debug("Connection GET :  oauth_token  ProviderId [ " + providerId + " ] ");
	...
	public RedirectView oauth2Callback(@PathVariable String providerId,
		...
		logger.debug("Connection GET :  code  ProviderId [ " + providerId + " ] ");
	...
	public RedirectView removeConnection(@PathVariable String providerId,
			@PathVariable String providerUserId, NativeWebRequest request) {
		connectionRepository.removeConnection(new ConnectionKey(providerId,
				providerUserId));
		return connectionStatusRedirect(providerId, request);
	}
	...
	protected RedirectView connectionStatusRedirect(String providerId,
		...
		String path = "/connect/" + providerId
	...
	private String getPathExtension(HttpServletRequest request) {
		String fileName = WebUtils.extractFullFilenameFromUrlPath(request
				.getRequestURI());
	...
	private void addConnection(Connection<?> connection,
		...
		try {
			...
			request.setAttribute(DUPLICATE_CONNECTION_ATTRIBUTE, e,
					RequestAttributes.SCOPE_SESSION);
		}
	}
	...
	private List<ConnectInterceptor<?>> interceptingConnectionsTo(
		...
		List<ConnectInterceptor<?>> typedInterceptors = interceptors
				.get(serviceType);
