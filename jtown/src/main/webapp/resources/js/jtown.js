							...
							.bind(
									...
									function() {
										...
										inner.find('input[name=j_username]')
												.attr('id', 'j_username');
										inner.find('input[name=j_password]')
												.attr('id', 'j_password');
										$.smartPop.open({
											width : 420,
											height : 330,
										...
										setTimeout(
												"$(function() { $('#j_username, #j_password').placeholder(); });",
												0);
										inner.find('#j_username')
												.attr('id', '');
										inner.find('#j_password')
												.attr('id', '');
									});

					$('.jt-header-nav-interestCategory-li').unbind(
							'mouseover mouseout');
					$('.jt-header-nav-interestCategory-li').bind(
							'mouseover mouseout',
							function(event) {
								var $interest = $(this).find(
										'.jt-header-nav-interest');
...
jtown.header.addInterest = function(pn) {
	...
	var url = contextPath + 'ajax/getNavInterest.jt', json = {
		'categoryPn' : pn
	};
	...
	$
			.postJSON(
					...
					json,
					function(data) {
						var interestSections = data.interestSections, interestCategories = data.interestCategories, pn = data.pn;

						var innerHtml = '';
						innerHtml += '<div id="jt-addInterest-wrap" data-categoryPn="'
								+ pn
								+ '" data-categoryLen="'
						...
						for ( var i = 0, len = interestSections.length; i < len; i++) {
							...
							innerHtml += '<a href="#none" class="jt-interestSection-click" data-sectionPn="'
									+ interestSection.sectionPn
									+ '">'
									+ interestSection.name + '</a>&nbsp;';
							if (i % 5 == 4) {
								innerHtml += '<br/>';
							}
						}
...
jtown.header.addInterestSync = function() {
	...
	$('#jt-addInterest-leftArrow').bind(
			'click',
			function() {
				var categoryPn = Number($('#jt-addInterest-wrap').attr(
						'data-categoryPn')) - 1;
				jtown.header.addInterest(categoryPn);
			});

	$('#jt-addInterest-rightArrow').unbind('click');
	$('#jt-addInterest-rightArrow').bind(
			'click',
			function() {
				var categoryPn = Number($('#jt-addInterest-wrap').attr(
						'data-categoryPn')) + 1;
	...
	$('.jt-interestSection-click').bind(
			...
			function() {
				...
				jtown.header.insertInterestSectionPn(categoryPn, sectionPn,
						sectionName);
			});
};

jtown.header.insertInterestSectionPn = function(categoryPn, sectionPn,
		sectionName) {
...
jtown.header.syncNavMove = function() {
	...
	$('.jt-header-nav-right')
			.bind(
					'click',
					function() {
						var scrollWidth = Number($('.jt-header-nav-scroll')
								.width()), allWidth = Number($(
								'.jt-header-nav-all').width()), gapWith = scrollWidth
								- allWidth, allLeftText = $(
								'.jt-header-nav-all').css('left'), allLeftWidth = Number(allLeftText
								.replace('px', '')), moveWidth = Number(0);
	...
	$('.jt-header-nav-left')
			.bind(
					...
					function() {
						var scrollWidth = Number($('.jt-header-nav-scroll')
								.width()), allLeftText = $('.jt-header-nav-all')
								.css('left'), allLeftWidth = Number(allLeftText
								.replace('px', '')), moveWidth = Number(0);
...
jtown.header.navInterestDelete = function(me) {
	var $parent = me.parents('.jt-header-nav-down'), cpn = $parent
			.attr('data-cpn'), spn = $parent.attr('data-spn');
	var url = contextPath + 'ajax/navInterestDelete.jt', json = {
		'categoryPn' : cpn,
		'sectionPn' : spn
	};
...
jtown.home.clickShop = function(spn) {
	var url = contextPath + 'ajax/clickShop.jt', json = {
		'sellerPn' : spn
	};
	...
	$.postJSON(url, json, function(count) {
		if (!nullValueCheck(count.message)) {
			...
			if (crudType == 'insert') {
				...
				$('#love-expand-image-' + spn).addClass(
						'jt-home-shop-love-click');
				$('#love-expand-' + spn).addClass(
						'jt-home-shop-love-text-click');
			} else if (crudType == 'delete') {
				$('#love-image-' + spn).removeClass('jt-home-shop-love-click');
				$('#love-' + spn).removeClass('jt-home-shop-love-text-click');
				$('#love-expand-image-' + spn).removeClass(
						'jt-home-shop-love-click');
				$('#love-expand-' + spn).removeClass(
						'jt-home-shop-love-text-click');
			}
		}
	});
...
jtown.home.html = function(data) {
	...
	for ( var i = 0, len = jtownUsers.length; i < len; i++) {
		...
		for ( var j = 0, jLen = mainImages.length; j < jLen; j++) {
			...
			var imageSrc = contextPath + 'resources/uploadImage/'
					+ ((mainImage == null) ? '8.jpg' : mainImage);
			imageHtml += '<img alt="" src="' + imageSrc + '" title="'
					+ htmlChars(seller.name) + '"/>	';
		}
		...
		html += '		<header>';
		html += '			<a href="http://' + seller.shopUrl
				+ '" target="_blank" onclick="jtown.home.clickShop(\'' + spn
				+ '\')">' + htmlChars(seller.name) + '</a>';
		html += '		</header>';
		...
		html += '			<pre class="jt-home-shop-footer-text">'
				+ htmlChars(seller.notice) + '</pre>';
		html += '			<span class="jt-home-shop-footer-lastQuotationMark">"</span>';
		html += '		</div>';
		html += '		<ul class="jt-home-shop-content-fn">';
		html += '			<li>';
		html += '				<span class="jt-home-shop-view">VIEW</span>&nbsp;<span id="view-'
				+ spn
				+ '">'
				+ (nullValueCheck(seller.viewCount) ? 0 : seller.viewCount)
				+ '</span>';
		html += '			</li>';
		html += '			<li>';
		html += '				<span class="jt-home-shop-comment">COMMENT</span>&nbsp;<span id="comment-'
				+ spn
				+ '">'
				+ (nullValueCheck(seller.commentCount) ? 0
						: seller.commentCount) + '</span>';
		html += '			</li>';
		html += '			<li>';
		html += '				' + loveHotCount;
		html += '				<a href="#none" onclick="jtown.home.clickLove(\'' + spn
				+ '\');"id="love-image-' + spn + '" class="jt-home-shop-love '
				+ loveClick + '">♥</a>&nbsp;<span id="love-' + spn
				+ '" class="' + loveTextClick + '">'
				+ (nullValueCheck(seller.loveCount) ? 0 : seller.loveCount)
				+ '</span>';
		html += '				<span id="new-' + spn + '">';
		html += '					'
				+ ((seller.bannerDate != null && Number(seller.bannerDate) < 8) ? 'new'
						: '');
		html += '				</span>';
		html += '			</li>';
		html += '		</ul>';
